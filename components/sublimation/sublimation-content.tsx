...
import { SublimationTable } from "./sublimation-table"
import { SublimationDetail } from "./sublimation-detail"
...
import { getSublimacionStatus } from "@/lib/production-status"
...
const SUBLIMACION_ESTADOS = ["Pendiente", "Recibido", "Terminado"] as const

export function SublimationContent() {
  const { ordenes, isLoading, error, refreshOrdenes } = useSublimation()
  ...
  const filteredOrdenes = useMemo(() => {
    return ordenes.filter((o) => {
      // Filtro global por fecha objetivo del area (sfecha_objetivo_s).
      if (!matchesTargetDate(o.sfecha_objetivo_s, targetDateFilter)) return false
      if (
        filters.pedido &&
      ...
      if (
        ...
        !filters.estado.includes(getSublimacionStatus(o))
      )
        return false
      return true
    })
    ...
    <Card className="bg-white/70 backdrop-blur-sm">
      <CardHeader>
        <div className="flex items-center justify-between gap-2">
          <CardTitle className="flex items-center gap-2">
            <Flame className="size-5 text-icon-coral" />
            Area de Sublimacion
          </CardTitle>
      ...
      <CardContent className="space-y-4">
        <ModuleTabs
          area="Sublimacion"
          accentClass="text-icon-coral"
          resumenContent={<ModuleResumenCard areaKey="sublimacion" />}
          ordenesContent={
            <>
              {error && (
              ...
              {(ordenes.length > 0 || isLoading) && (
                <ProductionFilters
                  ...
                  clientes={clientes}
                  estadoLabel="Estado Sublimacion"
                  estadoOptions={SUBLIMACION_ESTADOS}
                  accentClass="text-icon-coral"
                />
              )}
              ...
              {!error && !isLoading && ordenes.length === 0 && (
                <div className="text-center py-12 text-muted-foreground">
                  No hay ordenes disponibles para el area de Sublimacion.
                </div>
              )}
